...
**自动兼容**：系统自动识别预训练模式（SimCLR 或 MoCo），无需手动配置

- ✅ **SimCLR 预训练权重** - 直接加载所有参数
- ✅ **MoCo 预训练权重** - 自动过滤动量编码器和队列参数

```bash
# 两种预训练模式的检查点均可直接使用
python scripts/finetune.py \
    --task circor_murmur \
    --pretrained checkpoints/moco_best.pt  # 或 simclr_best.pt
```
...
> 💡 技术细节见 [MoCo优化实现说明.md](./MoCo优化实现说明.md#下游微调兼容性)
...
PA-HCL 使用 WandB 进行实验监控，云端监控，功能完整，支持团队协作。
...
| `--wandb-project` | `PA-HCL` | WandB 项目名称 |
| `--wandb-entity` | `None` | WandB 用户/团队名 |
