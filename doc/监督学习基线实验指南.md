...
1. **验证编码器设计**: 评估编码器架构（CNN-Mamba, SinCNeXt-BM, ResNet-1D）在监督学习任务上的性能
...
此脚本会自动：
- 检查环境和数据
- 依次训练各种编码器架构（CNN-Mamba, SinCNeXt-BM, ResNet-1D 等）
- 生成结果对比
...
# 训练 SincNet-ECA-Mamba 编码器
python scripts/train_supervised_baseline.py \
    --task physionet2016 \
    --encoder-type sincnet_eca_mamba \
    --experiment-name physionet2016_sincnet_eca_mamba

# 训练 1D-ResNet34 编码器（简洁基线）
python scripts/train_supervised_baseline.py \
    --task physionet2016 \
    --encoder-type resnet34_1d \
    --experiment-name physionet2016_resnet34_1d
...
| 编码器类型 | 描述 | 参数量 | 特点 | 用途 |
|-----------|------|--------|------|------|
| `cnn_mamba` | CNN-Mamba 混合 | ~65M | 感受野大，支持长依赖 | 默认选择，性能平衡 |
| `sincnet_eca_mamba` | SinCNeXt-BM（限频+ECA+Mamba） | ~70M | 限频滤波，参数高效 | 频域特征，微调友好 |
| `resnet34_1d` | 1D-ResNet34 简洁基线 | ~21M | 轻量级，无 Mamba | 快速验证，对比基线 |
...
model:
  # 选择编码器类型
  encoder_type: "cnn_mamba"  # 选项: cnn_mamba, sincnet_eca_mamba, resnet34_1d
  ...
  # SinCNeXt-BM 配置（encoder_type="sincnet_eca_mamba" 时使用）
  sinc_out_channels: 64
  local_dim: 128
  cycle_output_dim: 256
  
  # ResNet34-1D 配置（encoder_type="resnet34_1d" 时使用）
  # 固定架构，无需配置
...
- **推荐用 `cnn_mamba`**：默认配置，平衡性最好，适合大多数场景
- **用 `sincnet_eca_mamba`**：需要与预训练模型保持一致，或需要限频特性
- **用 `resnet34_1d`**：快速实验、对比基线、参数量受限的场景
