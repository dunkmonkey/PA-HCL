# PA-HCL 项目使用文档

本文档提供 PA-HCL 项目的详细使用指南，包括环境搭建、数据准备、模型训练、评估和部署。

---

## 目录

1. [环境配置](#环境配置)
2. [数据准备](#数据准备)
3. [配置文件说明](#配置文件说明)
4. [预训练](#预训练)
5. [下游任务微调](#下游任务微调)
6. [模型评估](#模型评估)
7. [消融实验](#消融实验)
8. [常见问题](#常见问题)
9. [最佳实践](#最佳实践)

---

## 环境配置

### 系统要求

- **操作系统**：Linux (推荐 Ubuntu 20.04+) / macOS / Windows (WSL2)
- **Python**：3.10+
- **GPU**：NVIDIA GPU with CUDA 11.8+ (推荐)
- **内存**：至少 16GB RAM
- **存储**：至少 50GB 可用空间

### 步骤 1: 克隆仓库

```bash
git clone https://github.com/dunkmonkey/PA-HCL.git
cd PA-HCL
```

### 步骤 2: 创建虚拟环境

#### 使用 Conda（推荐）

```bash
# 创建新环境
conda create -n pahcl python=3.10
conda activate pahcl
```

#### 使用 venv

```bash
python3.10 -m venv venv
source venv/bin/activate  # Linux/macOS
# 或
.\venv\Scripts\activate  # Windows
```

### 步骤 3: 安装 PyTorch

根据您的 CUDA 版本安装 PyTorch：

```bash
# CUDA 11.8
pip install torch==2.1.2 torchaudio==2.1.2 torchvision==0.16.2 \
    --index-url https://download.pytorch.org/whl/cu118

# CUDA 12.1
pip install torch==2.1.2 torchaudio==2.1.2 torchvision==0.16.2 \
    --index-url https://download.pytorch.org/whl/cu121

# CPU only (不推荐用于训练)
pip install torch==2.1.2 torchaudio==2.1.2 torchvision==0.16.2 \
    --index-url https://download.pytorch.org/whl/cpu
```

### 步骤 4: 安装项目依赖

```bash
# 安装核心依赖
pip install -r requirements.txt

# 以开发模式安装项目
pip install -e .
```

### 步骤 5: 验证安装

```bash
# 测试 PyTorch 是否正确安装
python -c "import torch; print(f'PyTorch: {torch.__version__}'); print(f'CUDA available: {torch.cuda.is_available()}')"

# 运行单元测试
pytest tests/ -v
```

**预期输出**：
```
PyTorch: 2.1.2
CUDA available: True
```

---

## 数据准备

### 数据组织结构

PA-HCL 要求原始数据按**受试者**组织（subject-wise），以确保训练/测试数据划分不泄露：

```
/root/autodl-tmp/data/raw/
├── subject_0001/
│   ├── rec_01.wav
│   ├── rec_02.wav
│   └── rec_03.wav
├── subject_0002/
│   ├── rec_01.wav
│   └── rec_02.wav
├── subject_0003/
│   └── ...
└── ...
```

### 数据格式要求

- **文件格式**：`.wav` (16-bit PCM 或浮点)
- **采样率**：原始采样率不限，会自动重采样到配置的采样率（默认 5000 Hz）
- **通道数**：单声道或立体声（立体声会自动转换为单声道）
- **时长**：每个录音至少包含 2-3 个完整心动周期

### 步骤 1: 准备原始数据

将您的心音数据按上述结构放置到 `/root/autodl-tmp/data/raw/` 目录。

**示例数据集**：
- CirCor DigiScope Dataset
- PhysioNet 2016 Challenge Dataset
- Pascal Challenge Dataset

### 步骤 2: 运行预处理

```bash
python scripts/preprocess.py --config configs/default.yaml
```

**预处理流程**：
1. 加载每个 `.wav` 文件
2. 重采样到目标采样率（默认 5000 Hz）
3. 带通滤波（25-400 Hz，去除噪声）
4. 检测心动周期（基于能量包络）
5. 切分并保存每个心动周期为独立文件

**输出结构**：
```
/root/autodl-tmp/data/processed/
├── subject_0001/
│   ├── rec_01/
│   │   ├── cycle_000.npy
│   │   ├── cycle_001.npy
│   │   ├── cycle_002.npy
│   │   └── ...
│   ├── rec_02/
│   │   └── ...
│   └── metadata.json
├── subject_0002/
│   └── ...
└── statistics.json  # 数据集统计信息
```

### 步骤 3: 验证预处理结果

```bash
# 检查处理后的数据
python -c "
from pathlib import Path
import numpy as np

processed_dir = Path('/root/autodl-tmp/data/processed')
cycle_files = list(processed_dir.rglob('*.npy'))
print(f'Total cycles: {len(cycle_files)}')

# 检查第一个周期
if cycle_files:
    cycle = np.load(cycle_files[0])
    print(f'Cycle shape: {cycle.shape}')
    print(f'Cycle dtype: {cycle.dtype}')
    print(f'Cycle range: [{cycle.min():.3f}, {cycle.max():.3f}]')
"
```

**预期输出**：
```
Total cycles: 15234
Cycle shape: (4000,)
Cycle dtype: float32
Cycle range: [-0.982, 0.945]
```

---

## 配置文件说明

PA-HCL 使用 YAML 配置文件管理所有超参数。主要配置文件：

### `configs/default.yaml` - 默认配置

包含所有可配置参数的默认值。

**关键参数**：

```yaml
# 数据配置
data:
  raw_dir: "./data/raw"              # 原始数据目录
  processed_dir: "./data/processed"  # 处理后数据目录
  sample_rate: 5000                  # 采样率 (Hz)
  num_substructures: 4               # 子结构数量 K
  
  cycle:
    min_duration: 0.4                # 最小心动周期时长 (秒)
    max_duration: 1.5                # 最大心动周期时长 (秒)
    target_length: 4000              # 目标长度 (采样点数)

# 模型配置
model:
  encoder:
    type: "cnn_mamba"                # 编码器类型
    cnn:
      channels: [32, 64, 128, 256]   # CNN 通道数
      kernel_sizes: [7, 5, 5, 3]     # 卷积核大小
      strides: [2, 2, 2, 2]          # 下采样步幅
    mamba:
      d_model: 256                   # Mamba 隐藏维度
      n_layers: 4                    # Mamba 层数
      d_state: 16                    # 状态维度

# 损失配置
loss:
  temperature: 0.07                  # InfoNCE 温度
  lambda_cycle: 1.0                  # 周期级损失权重
  lambda_sub: 1.0                    # 子结构级损失权重

# 训练配置
training:
  pretrain:
    epochs: 100                      # 训练轮数
    batch_size: 64                   # 批次大小
    optimizer:
      lr: 1.0e-3                     # 学习率
      weight_decay: 1.0e-4           # 权重衰减
```

### `configs/pretrain.yaml` - 预训练配置

专门用于自监督预训练的配置，继承自 `default.yaml`。

### `configs/finetune.yaml` - 微调配置

用于下游任务微调的配置。

```yaml
training:
  finetune:
    epochs: 50
    batch_size: 32
    encoder_lr: 1.0e-4               # 编码器学习率（较小）
    head_lr: 1.0e-3                  # 分类头学习率（较大）
    freeze_encoder: false            # 是否冻结编码器
```

### `configs/ablation.yaml` - 消融实验配置

用于消融实验的配置。

### 修改配置

有三种方式修改配置：

#### 方法 1: 直接编辑 YAML 文件

```bash
vim configs/pretrain.yaml
```

#### 方法 2: 命令行覆盖

```bash
python scripts/pretrain.py \
    --config configs/pretrain.yaml \
    --batch-size 128 \
    --learning-rate 2e-3
```

#### 方法 3: 创建自定义配置

```bash
cp configs/pretrain.yaml configs/my_config.yaml
vim configs/my_config.yaml
python scripts/pretrain.py --config configs/my_config.yaml
```

---

## 预训练

预训练是 PA-HCL 的核心步骤，从大量无标注心音数据中学习通用表示。

### 单 GPU 训练

```bash
python scripts/pretrain.py \
    --config configs/pretrain.yaml \
    --experiment-name my_pretrain \
    --output-dir outputs
```

### 多 GPU 训练（分布式数据并行）

```bash
# 使用 4 个 GPU
torchrun --nproc_per_node=4 scripts/pretrain.py \
    --config configs/pretrain.yaml \
    --distributed \
    --experiment-name my_pretrain_ddp
```

### 使用 Weights & Biases 记录

```bash
python scripts/pretrain.py \
    --config configs/pretrain.yaml \
    --use-wandb \
    --wandb-project PA-HCL \
    --experiment-name my_pretrain_wandb
```

### 从检查点恢复

```bash
python scripts/pretrain.py \
    --config configs/pretrain.yaml \
    --resume outputs/my_pretrain/epoch_50.pt
```

### 预训练输出

```
outputs/
└── my_pretrain/
    ├── config.yaml              # 保存的配置
    ├── train.log                # 训练日志
    ├── events.out.tfevents.*    # TensorBoard 日志
    ├── epoch_10.pt              # 每 10 轮保存的检查点
    ├── epoch_20.pt
    ├── ...
    ├── last_model.pt            # 最后一轮的模型
    └── best_model.pt            # 验证集上最佳模型（如有验证集）
```

### 监控训练

#### 使用 TensorBoard

TensorBoard 是默认启用的监控工具，训练时会自动记录日志。

**启动 TensorBoard**：

```bash
# 监控单个实验
tensorboard --logdir outputs/my_pretrain

# 监控所有实验（对比多个运行）
tensorboard --logdir outputs

# 指定端口
tensorboard --logdir outputs/my_pretrain --port 6007
```

在浏览器打开 `http://localhost:6006`

**可视化内容**：
- **Loss curves**：cycle_loss, sub_loss, total_loss
- **Learning rate schedule**：学习率变化曲线
- **Gradient norms**：梯度范数（检测梯度爆炸/消失）
- **Training speed**：每步训练时间

**TensorBoard 技巧**：

1. **实时监控**：TensorBoard 会自动刷新，无需重启
2. **平滑曲线**：使用左侧滑块调整平滑度
3. **对比实验**：在 `logdir` 指定父目录可同时查看多个实验
4. **下载数据**：点击左下角下载按钮导出 CSV 数据

#### 使用 W&B

Weights & Biases 提供更强大的实验跟踪、可视化和协作功能。

**步骤 1：安装和登录**

```bash
# 安装 wandb
pip install wandb

# 登录（首次使用）
wandb login
```

访问 https://wandb.ai/authorize 获取 API key。

**步骤 2：启用 W&B 记录**

在训练命令中添加 W&B 相关参数：

```bash
# 预训练
python scripts/pretrain.py \
    --config configs/pretrain.yaml \
    --use-wandb \
    --wandb-project PA-HCL \
    --wandb-entity your-team \
    --experiment-name my_pretrain_wandb

# 微调
python scripts/finetune.py \
    --config configs/finetune.yaml \
    --pretrained outputs/pretrain/best_model.pt \
    --use-wandb \
    --wandb-project PA-HCL-Finetune \
    --experiment-name downstream_task
```

**参数说明**：
- `--use-wandb`：启用 W&B 记录
- `--wandb-project`：项目名称（用于组织实验）
- `--wandb-entity`：团队/用户名（可选）
- `--experiment-name`：运行名称

**步骤 3：查看 W&B 面板**

训练开始后，终端会显示面板链接：
```
wandb: 🚀 View run at https://wandb.ai/your-username/PA-HCL/runs/xxxxx
```

访问链接查看：
- 实时训练曲线
- 系统资源监控（GPU、CPU、内存）
- 超参数配置
- 模型架构
- 日志输出

**W&B 高级用法**：

1. **添加标签和分组**：
   ```bash
   python scripts/pretrain.py \
       --config configs/pretrain.yaml \
       --use-wandb \
       --wandb-project PA-HCL \
       --experiment-name encoder_cnn_mamba \
       --wandb-tags "ablation,encoder,mamba" \
       --wandb-group "encoder_comparison"
   ```

2. **离线模式**（网络受限时）：
   ```bash
   export WANDB_MODE=offline
   python scripts/pretrain.py --use-wandb ...
   
   # 训练完成后同步
   wandb sync outputs/my_pretrain/wandb
   ```

3. **恢复运行**：
   ```bash
   python scripts/pretrain.py \
       --resume outputs/my_pretrain/epoch_50.pt \
       --wandb-resume-id xxxxx  # 从 W&B 面板获取 run ID
   ```

**同时使用 TensorBoard 和 W&B**：

两个工具可以同时启用，互不冲突：

```bash
# 训练会同时记录到两个平台
python scripts/pretrain.py \
    --config configs/pretrain.yaml \
    --use-wandb \
    --wandb-project PA-HCL \
    --experiment-name my_experiment

# 在另一个终端启动 TensorBoard
tensorboard --logdir outputs/my_experiment
```

**监控的关键指标**：

预训练阶段：
- `cycle_loss`：周期级对比损失（应逐渐下降）
- `sub_loss`：子结构级对比损失（应逐渐下降）
- `total_loss`：总损失 = λ_cycle × cycle_loss + λ_sub × sub_loss
- `learning_rate`：学习率变化（warmup + cosine decay）
- `grad_norm`：梯度范数（过大表示梯度爆炸）

微调阶段：
- `train_loss` / `val_loss`：训练/验证损失
- `train_acc` / `val_acc`：训练/验证准确率
- `auroc`：ROC 曲线下面积
- `auprc`：PR 曲线下面积
- `f1_score`：F1 分数

**故障排查**：

1. **TensorBoard 无法访问**：
   ```bash
   # 检查是否已安装
   pip install tensorboard
   
   # 检查端口是否被占用
   tensorboard --logdir outputs --port 6007
   ```

2. **W&B 登录失败**：
   ```bash
   # 重新登录
   wandb login --relogin
   
   # 或直接使用 API key
   wandb login --relogin YOUR_API_KEY
   ```

3. **W&B 同步慢**：
   ```bash
   # 减少日志频率（在配置文件中）
   training:
     log_interval: 100  # 每 100 步记录一次（默认 10）
   ```

### 预训练最佳实践

1. **批次大小**：
   - 单 GPU：64-128（取决于 GPU 内存）
   - 多 GPU：每个 GPU 64，总 effective batch size = 64 × N_GPU

2. **学习率**：
   - 基准：1e-3
   - 使用大批次时：lr = base_lr × sqrt(batch_size / 64)
   - 示例：batch_size=256 → lr=2e-3

3. **训练时长**：
   - 小数据集（<10k 样本）：50-100 epochs
   - 大数据集（>100k 样本）：100-200 epochs
   - 观察验证集损失，出现过拟合时提前停止

4. **混合精度**：
   - 推荐开启（`use_amp: true`），可节省约 50% 内存
   - 某些硬件（V100, A100）可获得 2-3x 加速

---

## 下游任务微调

预训练完成后，在特定下游任务上微调模型。

### 任务类型

#### 1. 线性评估（Linear Evaluation）

**特点**：冻结编码器，只训练分类头

**用途**：评估预训练表示的质量

```bash
python scripts/finetune.py \
    --config configs/finetune.yaml \
    --pretrained outputs/my_pretrain/best_model.pt \
    --linear-eval \
    --num-classes 2 \
    --experiment-name linear_eval
```

#### 2. 全量微调（Full Fine-tuning）

**特点**：编码器和分类头都参与训练

**用途**：追求最佳性能

```bash
python scripts/finetune.py \
    --config configs/finetune.yaml \
    --pretrained outputs/my_pretrain/best_model.pt \
    --num-classes 2 \
    --experiment-name full_finetune
```

#### 3. 小样本学习（Few-shot Learning）

**特点**：仅使用少量标注数据

**用途**：评估预训练模型的数据效率

```bash
# 使用 10% 训练数据
python scripts/finetune.py \
    --config configs/finetune.yaml \
    --pretrained outputs/my_pretrain/best_model.pt \
    --few-shot \
    --shot-ratio 0.1 \
    --num-classes 2 \
    --experiment-name few_shot_10pct

# 使用 1% 训练数据
python scripts/finetune.py \
    --config configs/finetune.yaml \
    --pretrained outputs/my_pretrain/best_model.pt \
    --few-shot \
    --shot-ratio 0.01 \
    --num-classes 2 \
    --experiment-name few_shot_1pct
```

### 准备下游数据

下游任务需要带标签的数据：

```
data/downstream/
├── train.csv
├── val.csv
└── test.csv
```

**CSV 格式**：
```csv
file_path,label
data/raw/subject_0001/rec_01.wav,0
data/raw/subject_0001/rec_02.wav,1
data/raw/subject_0002/rec_01.wav,0
...
```

### 微调配置

**关键参数**：

```yaml
# configs/finetune.yaml
data:
  train_csv: "data/downstream/train.csv"
  val_csv: "data/downstream/val.csv"
  test_csv: "data/downstream/test.csv"
  num_classes: 2                    # 分类类别数

training:
  finetune:
    epochs: 50
    batch_size: 32
    encoder_lr: 1.0e-4              # 编码器学习率（小）
    head_lr: 1.0e-3                 # 分类头学习率（大）
    freeze_encoder: false           # 线性评估时设为 true
    
    # 早停
    early_stopping:
      patience: 10                  # 验证集无改善时的等待轮数
      min_delta: 0.001              # 最小改善幅度
```

### 微调输出

```
outputs/
└── full_finetune/
    ├── config.yaml
    ├── train.log
    ├── best_model.pt               # 验证集上最佳模型
    ├── last_model.pt
    ├── confusion_matrix.png        # 混淆矩阵
    └── metrics.json                # 评估指标
```

---

## 模型评估

### 评估指标

PA-HCL 支持以下评估指标：

- **准确率 (Accuracy)**：整体分类正确率
- **AUROC (Area Under ROC)**：ROC 曲线下面积
- **AUPRC (Area Under Precision-Recall)**：PR 曲线下面积
- **F1-score**：精确率和召回率的调和平均
- **Precision**：精确率
- **Recall**：召回率
- **Specificity**：特异性

### 运行评估

```bash
python scripts/evaluate.py \
    --checkpoint outputs/full_finetune/best_model.pt \
    --data-csv data/downstream/test.csv \
    --config configs/finetune.yaml \
    --output-dir outputs/evaluation \
    --confusion-matrix \
    --roc-curve \
    --pr-curve
```

### 评估输出

```
outputs/evaluation/
├── metrics.json                 # 所有指标的 JSON 文件
├── confusion_matrix.png         # 混淆矩阵可视化
├── roc_curve.png                # ROC 曲线
├── pr_curve.png                 # Precision-Recall 曲线
└── predictions.csv              # 每个样本的预测结果
```

**metrics.json 示例**：
```json
{
  "accuracy": 0.8523,
  "auroc": 0.9234,
  "auprc": 0.8967,
  "f1_score": 0.8421,
  "precision": 0.8654,
  "recall": 0.8198,
  "specificity": 0.8734,
  "confusion_matrix": [[456, 67], [89, 388]]
}
```

### 批量评估

评估多个检查点：

```bash
# 评估脚本
for ckpt in outputs/full_finetune/epoch_*.pt; do
    echo "Evaluating $ckpt"
    python scripts/evaluate.py \
        --checkpoint "$ckpt" \
        --data-csv data/downstream/test.csv \
        --output-dir "outputs/evaluation/$(basename $ckpt .pt)"
done
```

### 跨数据集评估

评估模型的泛化能力：

```bash
# 在不同数据集上评估
python scripts/evaluate.py \
    --checkpoint outputs/full_finetune/best_model.pt \
    --data-csv data/external_dataset/test.csv \
    --config configs/finetune.yaml \
    --output-dir outputs/evaluation_external
```

---

## 消融实验

消融实验用于分析不同组件的贡献。

### 运行所有消融

```bash
python scripts/ablation.py \
    --config configs/ablation.yaml \
    --all \
    --output-dir outputs/ablation
```

### 单独运行消融

#### 1. 层次结构消融

验证周期级和子结构级对比的作用：

```bash
python scripts/ablation.py \
    --config configs/ablation.yaml \
    --hierarchy-ablation \
    --output-dir outputs/ablation/hierarchy
```

**实验配置**：
- **仅周期级**：`lambda_cycle=1.0, lambda_sub=0.0`
- **仅子结构级**：`lambda_cycle=0.0, lambda_sub=1.0`
- **完整方法**：`lambda_cycle=1.0, lambda_sub=1.0`

#### 2. 编码器架构消融

对比不同编码器的性能：

```bash
python scripts/ablation.py \
    --config configs/ablation.yaml \
    --encoder-ablation \
    --output-dir outputs/ablation/encoder
```

**实验配置**：
- **CNN-only**：`encoder_type="cnn_only"`
- **CNN-Transformer**：`encoder_type="cnn_transformer"`
- **CNN-Mamba**：`encoder_type="cnn_mamba"`（本文）

#### 3. 子结构数量消融

探索最优的子结构划分：

```bash
python scripts/ablation.py \
    --config configs/ablation.yaml \
    --num-substructures-ablation \
    --output-dir outputs/ablation/num_subs
```

**实验配置**：
- **K=2**：粗粒度划分
- **K=4**：默认配置
- **K=6**：细粒度划分

#### 4. 损失权重消融

寻找最优的损失权重组合：

```bash
python scripts/ablation.py \
    --config configs/ablation.yaml \
    --loss-weight-ablation \
    --output-dir outputs/ablation/loss_weights
```

**实验配置**：
- `(λ_cycle, λ_sub) = (1.0, 0.0), (1.0, 0.5), (1.0, 1.0), (0.5, 1.0), (0.0, 1.0)`

### 消融实验输出

```
outputs/ablation/
├── hierarchy/
│   ├── cycle_only/
│   │   ├── pretrain/
│   │   └── finetune/
│   ├── sub_only/
│   │   ├── pretrain/
│   │   └── finetune/
│   └── full/
│       ├── pretrain/
│       └── finetune/
├── encoder/
│   ├── cnn_only/
│   ├── cnn_transformer/
│   └── cnn_mamba/
└── summary.csv                  # 所有消融结果汇总
```

**summary.csv 示例**：
```csv
experiment,encoder,lambda_cycle,lambda_sub,K,accuracy,auroc,f1
cycle_only,cnn_mamba,1.0,0.0,4,0.8234,0.8923,0.8156
sub_only,cnn_mamba,0.0,1.0,4,0.8156,0.8745,0.8067
full,cnn_mamba,1.0,1.0,4,0.8523,0.9234,0.8421
...
```

---

## 常见问题

### Q1: 内存不足 (Out of Memory)

**症状**：`RuntimeError: CUDA out of memory`

**解决方案**：

1. **减小批次大小**：
   ```yaml
   training:
     pretrain:
       batch_size: 32  # 从 64 减到 32
   ```

2. **启用混合精度**：
   ```yaml
   training:
     use_amp: true
   ```

3. **使用梯度累积**：
   ```yaml
   training:
     batch_size: 16
     gradient_accumulation_steps: 4  # effective batch_size = 16 × 4 = 64
   ```

4. **减少模型大小**：
   ```yaml
   model:
     mamba:
       d_model: 128     # 从 256 减到 128
       n_layers: 2      # 从 4 减到 2
   ```

### Q2: 训练速度慢

**解决方案**：

1. **启用混合精度**（AMP）
2. **增加 num_workers**：
   ```yaml
   training:
     num_workers: 8  # 数据加载并行度
   ```
3. **使用多 GPU**：
   ```bash
   torchrun --nproc_per_node=4 scripts/pretrain.py --distributed
   ```
4. **数据缓存到内存**：
   ```python
   dataset = PCGPretrainDataset(..., cache_in_memory=True)
   ```

### Q3: 预处理后周期数量不符合预期

**可能原因**：
- 音频质量差，信噪比低
- 心动周期检测阈值不合适

**解决方案**：

调整预处理参数：
```yaml
data:
  cycle:
    min_duration: 0.3  # 降低最小时长
    max_duration: 2.0  # 增加最大时长
  filter:
    min_snr_db: 5.0    # 降低 SNR 阈值（但可能引入噪声）
```

### Q4: 模型性能不佳

**诊断步骤**：

1. **检查数据质量**：
   ```bash
   python -c "
   import numpy as np
   from pathlib import Path
   
   cycles = [np.load(f) for f in Path('data/processed').rglob('*.npy')[:100]]
   print('Mean:', np.mean([c.mean() for c in cycles]))
   print('Std:', np.mean([c.std() for c in cycles]))
   "
   ```

2. **可视化样本**：
   ```python
   import matplotlib.pyplot as plt
   import numpy as np
   
   cycle = np.load('data/processed/subject_0001/rec_01/cycle_000.npy')
   plt.plot(cycle)
   plt.title('Heart Sound Cycle')
   plt.xlabel('Sample')
   plt.ylabel('Amplitude')
   plt.savefig('cycle_visualization.png')
   ```

3. **检查训练日志**：
   - Loss 是否下降？
   - 是否出现梯度爆炸/消失？

4. **尝试不同的超参数**：
   - 学习率：1e-4, 5e-4, 1e-3, 2e-3
   - 温度系数：0.05, 0.07, 0.1
   - 损失权重：不同的 λ 组合

### Q5: Mamba 安装失败

**症状**：`ERROR: Failed building wheel for mamba-ssm`

**原因**：Mamba 需要 CUDA 和编译环境

**解决方案**：

1. **确保安装了 CUDA toolkit**：
   ```bash
   nvcc --version
   ```

2. **安装编译依赖**：
   ```bash
   # Ubuntu
   sudo apt-get install build-essential
   
   # CentOS
   sudo yum groupinstall "Development Tools"
   ```

3. **使用预编译版本**（如可用）：
   ```bash
   pip install mamba-ssm --find-links https://github.com/state-spaces/mamba/releases
   ```

4. **回退到 Transformer**：
   ```yaml
   model:
     encoder:
       type: "cnn_transformer"  # 不需要 Mamba
   ```

### Q6: 分布式训练报错

**症状**：`RuntimeError: Default process group has not been initialized`

**解决方案**：

确保使用 `torchrun` 启动：
```bash
# 正确
torchrun --nproc_per_node=4 scripts/pretrain.py --distributed

# 错误
python scripts/pretrain.py --distributed  # 缺少 torchrun
```

### Q7: 如何处理不平衡数据集？

**解决方案**：

1. **类别加权**：
   ```python
   # 在微调时
   class_weights = torch.tensor([1.0, 5.0])  # 正常:异常 = 5:1
   criterion = nn.CrossEntropyLoss(weight=class_weights)
   ```

2. **重采样**：
   ```python
   from torch.utils.data import WeightedRandomSampler
   
   # 计算样本权重
   class_counts = [5000, 1000]  # 正常:5000, 异常:1000
   weights = [1.0/c for c in class_counts]
   sample_weights = [weights[label] for label in labels]
   
   sampler = WeightedRandomSampler(sample_weights, len(sample_weights))
   loader = DataLoader(dataset, sampler=sampler, ...)
   ```

3. **数据增强**：
   对少数类应用更强的增强

---

## 最佳实践

### 1. 数据准备

✅ **推荐做法**：
- 按受试者组织数据，避免数据泄露
- 预处理前检查音频质量
- 使用一致的采样率
- 保留原始数据备份

❌ **避免**：
- 混合不同来源的数据而不标注
- 忽略音频质量评估
- 删除原始数据

### 2. 预训练

✅ **推荐做法**：
- 使用大批次（64-128）和混合精度
- 设置验证集监控训练
- 定期保存检查点
- 使用 TensorBoard/W&B 监控
- 设置随机种子保证可复现

❌ **避免**：
- 过早停止训练（至少 50 epochs）
- 忽略验证集性能
- 不保存配置文件

### 3. 微调

✅ **推荐做法**：
- 先尝试线性评估，再全量微调
- 使用较小的编码器学习率
- 设置早停避免过拟合
- 在多个随机种子下评估

❌ **避免**：
- 使用过大的学习率（导致灾难性遗忘）
- 在训练集上过拟合
- 只在单次运行下报告结果

### 4. 评估

✅ **推荐做法**：
- 使用多种指标（Accuracy, AUROC, F1）
- 报告置信区间（多次运行）
- 绘制混淆矩阵和 ROC 曲线
- 进行跨数据集评估

❌ **避免**：
- 只报告准确率
- 在验证集上调参后在测试集报告
- 忽略类别不平衡的影响

### 5. 实验管理

✅ **推荐做法**：
- 使用有意义的实验名称
- 版本控制配置文件
- 记录每次实验的超参数
- 使用实验跟踪工具（W&B）

❌ **避免**：
- 覆盖之前的实验结果
- 不记录随机种子
- 手动管理实验

### 6. 代码开发

✅ **推荐做法**：
- 运行单元测试
- 使用代码格式化工具（black, isort）
- 添加类型注解
- 编写文档字符串

❌ **避免**：
- 直接修改核心代码而不测试
- 忽略代码风格
- 提交未测试的代码

---

## 高级用法

### 自定义数据增强

创建新的增强变换：

```python
# src/data/transforms.py

class MyCustomTransform(Transform):
    def __init__(self, param1, param2, p=0.5):
        super().__init__(p=p)
        self.param1 = param1
        self.param2 = param2
    
    def apply(self, signal_data, sample_rate):
        # 实现您的增强逻辑
        augmented = ...
        return augmented

# 使用
from src.data.transforms import Compose, MyCustomTransform

transforms = Compose([
    MyCustomTransform(param1=..., param2=...),
    # ... 其他变换
])
```

### 自定义编码器

实现新的编码器架构：

```python
# src/models/encoder.py

class MyCustomEncoder(nn.Module):
    def __init__(self, **kwargs):
        super().__init__()
        # 定义您的网络层
        
    def forward(self, x):
        # 前向传播
        return features
    
    def get_substructure_features(self, x):
        # 返回中间特征（用于子结构对比）
        return mid_features

# 注册编码器
def build_encoder(encoder_type, **kwargs):
    if encoder_type == "my_custom":
        return MyCustomEncoder(**kwargs)
    # ...
```

### 自定义损失函数

```python
# src/losses/contrastive.py

class MyCustomLoss(nn.Module):
    def __init__(self, **kwargs):
        super().__init__()
        # 初始化参数
    
    def forward(self, z1, z2):
        # 计算损失
        loss = ...
        return loss
```

### 导出模型用于部署

```python
# 导出为 ONNX 格式
import torch
from src.models.pahcl import PAHCLModel

model = PAHCLModel(...)
model.load_state_dict(torch.load('best_model.pt'))
model.eval()

dummy_input = torch.randn(1, 1, 4000)
torch.onnx.export(
    model,
    dummy_input,
    "pahcl_model.onnx",
    input_names=['input'],
    output_names=['output'],
    dynamic_axes={'input': {0: 'batch_size'}, 'output': {0: 'batch_size'}}
)
```

### 使用预训练模型进行推理

```python
import torch
import numpy as np
from src.models.pahcl import PAHCLModel
from src.data.preprocessing import load_audio, bandpass_filter

# 加载模型
model = PAHCLModel(...)
checkpoint = torch.load('outputs/my_pretrain/best_model.pt')
model.load_state_dict(checkpoint['model_state_dict'])
model.eval()

# 加载音频
audio, sr = load_audio('test_audio.wav', target_sr=5000)
audio = bandpass_filter(audio, sr, low=25, high=400)

# 推理
with torch.no_grad():
    audio_tensor = torch.from_numpy(audio).unsqueeze(0).unsqueeze(0).float()
    output = model(audio_tensor)
    features = output['features']
    
print(f"Extracted features shape: {features.shape}")
```

---

## 附录

### A. 完整命令速查表

```bash
# 环境设置
conda create -n pahcl python=3.10
conda activate pahcl
pip install torch==2.1.2 torchaudio==2.1.2 --index-url https://download.pytorch.org/whl/cu118
pip install -r requirements.txt
pip install -e .

# 数据预处理
python scripts/preprocess.py --config configs/default.yaml

# 预训练
python scripts/pretrain.py --config configs/pretrain.yaml
torchrun --nproc_per_node=4 scripts/pretrain.py --config configs/pretrain.yaml --distributed

# 微调
python scripts/finetune.py --config configs/finetune.yaml --pretrained outputs/pretrain/best_model.pt --linear-eval
python scripts/finetune.py --config configs/finetune.yaml --pretrained outputs/pretrain/best_model.pt
python scripts/finetune.py --config configs/finetune.yaml --pretrained outputs/pretrain/best_model.pt --few-shot --shot-ratio 0.1

# 评估
python scripts/evaluate.py --checkpoint outputs/finetune/best_model.pt --data-csv data/test.csv

# 消融实验
python scripts/ablation.py --config configs/ablation.yaml --all

# 测试
pytest tests/ -v
pytest tests/ --cov=src --cov-report=html
```

### B. 配置参数完整列表

详见 `configs/default.yaml` 文件。

### C. 性能基准

**硬件环境**：
- GPU: NVIDIA A100 40GB
- CPU: 32 cores
- RAM: 128GB

**性能数据**：

| 操作 | 时间 | 吞吐量 |
|------|------|--------|
| 预处理 (10k 音频) | ~30 min | ~5.5 audio/sec |
| 预训练 (100 epochs, 50k cycles) | ~4 hours | ~350 cycles/sec |
| 微调 (50 epochs) | ~30 min | - |
| 推理 (单样本) | ~10 ms | ~100 samples/sec |

### D. 故障排查清单

遇到问题时，按此清单检查：

- [ ] PyTorch 和 CUDA 版本匹配？
- [ ] 数据格式正确？
- [ ] 配置文件路径正确？
- [ ] GPU 内存足够？
- [ ] 依赖包版本正确？
- [ ] 随机种子已设置？
- [ ] 日志文件中有错误信息？

---

## 联系与支持

- **GitHub Issues**: https://github.com/dunkmonkey/PA-HCL/issues
- **文档**: https://github.com/dunkmonkey/PA-HCL/wiki
- **讨论区**: https://github.com/dunkmonkey/PA-HCL/discussions

---

**文档版本**：v1.0  
**最后更新**：2026年1月18日
